...
class VGGVox(nn.Module):
    def __init__(self, nOut=1024, encoder_type='SAP', log_input=True, **kwargs):
        ...
        # Arsitektur VGG Klasik (Conv2D -> BatchNorm -> ReLU -> MaxPool)
        ...
        # Feature Extraction Internal (Raw Audio -> MelSpec)
    ...
    def forward(self, x):
        # [REVISI] Pastikan input (Batch, Time) sebelum masuk feature extractor
        if x.dim() == 3:
            x = x.squeeze(1)
...
def MainModel(**kwargs):
    # [REVISI] Adapter untuk Factory Pattern
    if 'embedding_dim' in kwargs and 'nOut' not in kwargs:
        kwargs['nOut'] = kwargs['embedding_dim']
    
    # Default nOut untuk VGGVox biasanya 1024 atau 512
